...
for (i in (lookback_months + 1):nrow(full_data_monthly)) {
  ...
  for (j in seq_along(industry_cols)) {
    ...
    lambda_shrink <- 0.2  # for example, 20% shrinkage
...
optimize_and_backtest <- function(target_market_beta_value) {
  ...
  # Now return BOTH results and weights
...
p_macroU <- ggplot(betas_all, aes(x = Date, y = Beta_MacroU, color = Portfolio)) +
  geom_line() +
  labs(title = "Realized Beta to Macro Uncertainty Over Time",
       x = "Date",
       y = "Beta to MacroU") +
  theme_minimal()
...
p_finU <- ggplot(betas_all, aes(x = Date, y = Beta_FinU, color = Portfolio)) +
  geom_line() +
  labs(title = "Realized Beta to Financial Uncertainty Over Time",
       x = "Date",
       y = "Beta to FinU") +
  theme_minimal()
...
p_tpu <- ggplot(betas_all, aes(x = Date, y = Beta_TPU, color = Portfolio)) +
  geom_line() +
  labs(title = "Realized Beta to Trade Policy Uncertainty Over Time",
       x = "Date",
       y = "Beta to TPU") +
  theme_minimal()

# Optional: Display all together if you want
library(patchwork)
...
# Assume "all_betas" is your combined rolling betas (already merged)
# If not, bind them: all_betas <- do.call(bind_rows, rolling_betas)
...
# Only keep the factors you want to plot
...
# Force dates to first of month
rolling_betas_list <- list()  # <<< You must create this empty list
# ========================
...
# Assuming you already have `betas_summary_all` and `market_betas`
# If not, make sure betas_summary_all has the average betas
...
# Assuming you have df_factors containing Date, MacroU, FinU, TPU, EPU

# 1. Regression of TPU on MacroU and FinU
model_tpu <- lm(TPU ~ MacroU + FinU, data = df_factors)
summary(model_tpu)

# 2. Regression of EPU on MacroU and FinU
model_epu <- lm(EPU ~ MacroU + FinU, data = df_factors)
summary(model_epu)
...
p <- ggplot(realized_betas_all, aes(x = Date, y = Realized_Beta, color = Portfolio)) +
  ...
  labs(
    ...
    linetype = "Reference Line"   # <- Label for linetype legend
  ) +
  scale_linetype_manual(values = c("Target Beta" = "dashed", "Average Realized Beta" = "dotted")) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, size = 16),
    legend.position = "bottom",    # <<< now showing legend nicely below
